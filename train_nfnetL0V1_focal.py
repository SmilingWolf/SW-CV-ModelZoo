...
if __name__ == "__main__":
    ...
    model = NFNetV1(
        in_shape=(dim, dim, 3), out_classes=2380, definition_name="L0", use_eca=False
    )
    model.load_weights(
        "checkpoints/NFNetL0V1-rmc-30-0.56855/NFNetL0V1-rmc-30-0.56855/variables/variables"
    )
    ...
    opt = SGD(learning_rate=0.5, momentum=0.9, nesterov=True)
    model.compile(optimizer=opt, loss=loss, metrics=[f1])

    arg = np.random.rand(miniBatch, dim, dim, 3)
    model.predict(arg)

    # resumeModel = 'NFNetL0V1-rmc-30-0.56855'
    # if resumeModel != '':
    ...
    rmc = tf.keras.callbacks.ModelCheckpoint(
        "checkpoints/NFNetL0V1-rmc-{epoch:02d}-{f1_score:.5f}/NFNetL0V1-rmc-{epoch:02d}-{f1_score:.5f}",
