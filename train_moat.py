...
if __name__ == "__main__":
    ...
    # Learning rate
    max_learning_rate = 4e-4 * multiplier
    ...
    block0_ws = None
    block1_ws = None
    # Partition into 4 non overlapping windows
    block2_ws = 10
    # Global attention in the last stage
    block3_ws = None
    window_sides = [block0_ws, block1_ws, block2_ws, block3_ws]
    ...
    wandb_run = wandb.init(
        ...
        entity="smilingwolf",
        config=train_config,
        name="%s_%s" % (definition_name, date_time),
        tags=[node_name],
    )
    ...
    rmc_loss = tf.keras.callbacks.ModelCheckpoint(
        "%s/checkpoints/%s_%s/variables/variables"
